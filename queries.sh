...
echo "$($PSQL "select distinct(name) from games inner join teams on games.winner_id=teams.team_id where round='Eighth-Final' and year=2014 union select distinct(name) from games inner join teams on games.opponent_id=teams.team_id where round='Eighth-Final' and year=2014 order by name asc")"
